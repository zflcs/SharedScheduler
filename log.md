...
1. 摘要中还是需要明确“Shared Scheduler”这个名字，以方便正文中的引用。结论还没有与摘要相呼应。
2. 引言
   1. 建议不要在正文中用“stackful coroutine”这个词，都称为“user-level thread”。
3. 2.1节的开头需要提及Rust、GO、Java、C++、Kotlin和python等语言都有并发编程机制的支持，在内核中比较关注Rust和Go语言。关于Goroutine的特征介绍需要学术论文类的文献引用来支持Goroutine是用户级线程等特征。
...
3. 文字流畅性
   ...
   1. 统一用词，用词要符合本专业通常的含义，尽量少用新词。
      * lightweight coroutine、coroutine, stackless coroutine
