...
1. 摘要中还是需要明确“Shared Scheduler”这个名字，以方便正文中的引用。（采纳并已修改）结论还没有与摘要相呼应（采纳，但结论需后续修改）。
2. 引言
   1. 建议不要在正文中用“stackful coroutine”这个词，都称为“user-level thread”。(采纳并已修改)
3. 2.1节的开头需要提及Rust、GO、Java、C++、Kotlin和python等语言都有并发编程机制的支持，在内核中比较关注Rust和Go语言。关于Goroutine的特征介绍需要学术论文类的文献引用来支持Goroutine是用户级线程等特征。（采纳并已修改）
...
1. 第四节：
   ...
   8. 4.3.2节：我认为同步和异步系统调用的内核实现函数都是对应一个协程；它们的区别在于，在执行完相应功能后通过用户态中断还是sret返回结果。
2. 第五节开头讨论的TcpServer的三个组件的名字不好。“Msg Server”容易引起误解，“Msg Sender”说成“Msg Send”会与“Msg Recv”更对应一些。
3. 文字流畅性
   1. 英文的表达不够简洁和流畅，还需要好好润色。（目前可先不处理）
   1. 统一用词，用词要符合本专业通常的含义，尽量少用新词。
      * lightweight coroutine、coroutine, stackless coroutine（采纳并已修改）
