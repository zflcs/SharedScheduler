...
1. 摘要中还是需要明确“Shared Scheduler”这个名字，以方便正文中的引用。（采纳并已修改）结论还没有与摘要相呼应（<font color="green">采纳，但结论需后续修改</front>）。
2. 引言
   1. 建议不要在正文中用“stackful coroutine”这个词，都称为“user-level thread”。(<font color="green">采纳并已修改</front>)
3. 2.1节的开头需要提及Rust、GO、Java、C++、Kotlin和python等语言都有并发编程机制的支持，在内核中比较关注Rust和Go语言。关于Goroutine的特征介绍需要学术论文类的文献引用来支持Goroutine是用户级线程等特征。（<font color="green">采纳并已修改</front>）
4. 建议在3.1节补充关于优先级位图的描述。
5. 图3中的状态名字还需要推敲。我认为，正在执行的协程被中断和异常打断后的处理与现在图中的表达有些出入（可能需要看反汇编出来的代码）；还有一个问题是，协程切换过程中被中断或异常打断时，协程的状态是如何变化的。需要对照代码再想想。
6. 对于图3中的协程状态图，还有一个需要考虑的问题。多核时会有空闲CPU和协程迁移的情况吗？如果有，会影响协程的状态吗？
7. 3.3节关于进程切换的描述中，有一个问题需要回答。用户态到内核态的切换过程是如何进行的？
8. 3.4节的共享调度器流程图还需要推敲。
   1. 在内核态时，如果没有就绪协程可以运行，CPU也应该执行idle协程（rCore-N中有这个吗？对于多核情况下，只有最后一个核执行idle协程，其他核应该也是暂停运行状态）；在用户态时，如果没有就绪协程，但有等待状态的协程，这时应该执行主动让权操作。
   2. “start”状态的进入出需要说明。可能进入“start”状态的事件有，有协程变成就绪状态、给进程分配了新的CPU核。另外，硬件中断和软件异常也会导致共享调度器代码的执行，这些情形应该出现在图4中。
...
2. 第五节开头讨论的TcpServer的三个组件的名字不好。“Msg Server”容易引起误解，“Msg Sender”说成“Msg Send”会与“Msg Recv”更对应一些。（<font color="green">采纳并已修改</front>）
3. 文字流畅性
   1. 英文的表达不够简洁和流畅，还需要好好润色。（目前可先不处理）
   1. 统一用词，用词要符合本专业通常的含义，尽量少用新词。
      * lightweight coroutine、coroutine, stackless coroutine（<font color="green">采纳并已修改</front>）
