...
1. 已投至 [ICPC 2024](https://icpc2024.hotcrp.com/) 

##### 20231014-向勇对中文稿的修改意见

1. 摘要：第二条贡献“对硬件线程的抽象”没有说清楚。
    - 第二条贡献改为将协程与内核异步 I/O 机制结合起来

2. 第一节第二段：作为要解决的问题，“执行顺序的不确定性”有些小，你也不解决这个问题。
    - 在 3.2 节加入了 SharedScheduler 对这个问题的处理。

3. 第一节第三段：这个问题是存在的和重要的，但你的论文并不解决它。可以换个角度来说，这些异步机制是你的论文借见和参考的内容。
    - 在第一节最后的结尾，更换了描述方式。

4. 3.3节的内容没有太说清楚。切换协程的优先级如何确定？什么时候执行它？目前的描述还需要推敲。
    - 在 3.4 节中已经增加描述

5. 图3画得不规范。“start”出来的箭头画成直的会清楚一些。
    - 已修改

6. 图5中，ktuc的表现更优秀。这说明高并发情况下协程才有优势。建议再测试一下更高并发度时的延时和吞吐率性能。
    - 更高连接数时，线程模型已经不能完成测试，而协程模型的吞吐量也没有增加，我认为是其他方面的原因，不是因为协程模型的问题。不如在文章中用文字描述。

7. 图7的两个图需要标一下CPU数目。
    - 已经注明

8. 摘要和结论还需要推敲。目前的表达与内容的对应关系不清晰。
    - 摘要已修改
...
1. 为什么是给服务端发送矩阵？是为了给服务端加计算负载，还是只是为了填充数据？
2. web server和内核的网络协议栈是怎么实现的？是基于现有的项目改造，还是自己重新实现的？如果是前者，原本的项目对多线程和协程的支持如何？
3. 这个测试看起来还是像一个microbench，能否构造一个基于现实应用的测试，比如redis, memcached, nginx，另外客户端应该有一些现成的工作负载数据，或者压力测试器，也可以把linux下的结果作为基线对比
4. 这个论文强调的是内核-用户的协调协程调度，那可对比的其实有四种组合，a. 内核协程-用户协程（理想），b. 内核协程-用户线程（兼容），c. 内核线程-用户协程（tokio等），d. 内核线程-用户线程（传统）。我理解现在测试里对比的应该是a和b，但从新内核架构的角度，可能a和cd的对比会更有说服力



##### 20230924-讨论实验结果

1. 关于使用 3 个 core 和使用 4 个 core时，两条线重叠的问题（目前的测试条件只能允许在负载较轻的条件下进行，在 3 和 4 个 core 的情况下，负载体现不出差距）



##### 20230916-讨论

1. 协程状态划分，根据 CPU 与栈资源占用进行划分
    ...
    - 不存在占用了锁之后被打算导致其他就绪协程不能运行的中间状态，某个协程在某个时刻一定处于上述几种状态

    - 运行挂起状态：若完全满足异步操作系统的特征，实际上不应该存在这种状态，一旦进程内还存在其他的就绪协程，CPU 应该能够一直工作。但由于目前的工作进度，尚未对中断、异常进行异步化改造，从而导致了这种运行挂起的状态，这种状态意味着需要在一段时间内占用栈。目前的代码中，跨越了特权级的中断和异常，会进行栈的切换，没有跨越特权级的中断与异常（内核中发生中断、用户态中断）则是复用当前栈，这两类情况无论如何，上一个协程都占用了栈，需要等待中断或异常处理完成后，才能继续运行或者让权，因此是处于这种运行挂起的状态。
    - 当协程使用同步系统调用时，那么这个协程也是处于这种运行挂起的状态，需要等待处理完成，才可以继续执行或让权。而协程使用异步系统调用时，尽管这时仍然需要进出内核一次，但它能够及时的让出栈给其他就绪协程执行，并不会长时间（上下文切换 + 系统调用分发的时间）占用栈不能运行，这个过程归结为 ==运行 -> 阻塞== 的状态转化，不属于 ==运行 -> 运行挂起状态==

2. 异步系统调用处理的过程：归属于系统调用分发协程

3. 可能改进的空间：

    - 同步和异步的接口都相同，在进行编译链接时再进行区分
    - 异步系统调用的改造尚不完全充分：极致的状态是对 ecall 指令进行改造，准备好参数后，直接向内核发送中断（或者以其他形式通知内核），从而不需要进入内核，而其他核上的内核则可以进行处理
    - 中断和异常进行异步化改造
...
[当前版本](https://github.com/zflcs/SharedScheduler/blob/55507ba107a0187a4f5abd6901a0606df49b8281/Shared Scheduler%2C  An Asynchronous Scheduler Shared by Userspace and Kernel Space..pdf)

###### 前三节的第二轮修改意见

1. 摘要中还是需要明确“Shared Scheduler”这个名字，以方便正文中的引用。（采纳并已修改）结论还没有与摘要相呼应（<font color="green">采纳，但结论需后续修改</font>）。
2. 引言
   1. 建议不要在正文中用“stackful coroutine”这个词，都称为“user-level thread”。(<font color="green">采纳并已修改</font>)
3. 2.1节的开头需要提及Rust、GO、Java、C++、Kotlin和python等语言都有并发编程机制的支持，在内核中比较关注Rust和Go语言。关于Goroutine的特征介绍需要学术论文类的文献引用来支持Goroutine是用户级线程等特征。（<font color="green">采纳并已修改</font>）
4. 建议在3.1节补充关于优先级位图的描述。(<font color="red">优先级位图的设计增加了 Executor 的复杂性，Executor 中只维护优先级字段，表示进程的优先级，只在内核中维护全局优先级位图，共享给用户进程只读权限，用于让权，在 3.1 中增加了 Executor 的描述。在 3.4 部分对于全局位图进行文字性的描述</font>)
5. 图3中的状态名字还需要推敲。我认为，正在执行的协程被中断和异常打断后的处理与现在图中的表达有些出入（可能需要看反汇编出来的代码）；还有一个问题是，协程切换过程中被中断或异常打断时，协程的状态是如何变化的。需要对照代码再想想。(<font color="red">协程状态增加了运行挂起状态，协程仍然处于运行状态，但是不在 CPU 上运行，等时机成熟可直接进入运行状态，可能需要进一步的讨论</font>)
6. 对于图3中的协程状态图，还有一个需要考虑的问题。多核时会有空闲CPU和协程迁移的情况吗？如果有，会影响协程的状态吗？(<font color="red">给同一个进程分配多个 CPU 时，存在协程迁移的情况，这种迁移前后不会改变协程的状态，CPU 对于协程的亲和性也不会由影响</font>)
7. 3.3节关于进程切换的描述中，有一个问题需要回答。用户态到内核态的切换过程是如何进行的？(<font color="red">这里对于协程概念上的界定尚不明确，需要讨论进一步的思考</font>)
8. 3.4节的共享调度器流程图还需要推敲。
   1. 在内核态时，如果没有就绪协程可以运行，CPU也应该执行idle协程（rCore-N中有这个吗？对于多核情况下，只有最后一个核执行idle协程，其他核应该也是暂停运行状态）；在用户态时，如果没有就绪协程，但有等待状态的协程，这时应该执行主动让权操作。(<font color="green">已采纳</font>)
   2. “start”状态的进入出需要说明。可能进入“start”状态的事件有，有协程变成就绪状态、给进程分配了新的CPU核。另外，硬件中断和软件异常也会导致共享调度器代码的执行，这些情形应该出现在图4中。(<font color="red">前半部分已采纳，后半部分的中断与异常，增加了复杂性，由于新的思考，暂时没有考虑</font>)
...
1. 第四节：
   ...
   5. 如果可能，可以把4.1节的示例代码换成socket的send和receive。这样与后续的内容更一致。(<font color="red">socket的send和receive也是调用的write和read，感觉这里强调异步系统调用在用户层面的统一，用write和read更能提现对类文件接口的通用性，暂时不修改.</font>)
   6. 4.2节的插图有些冗余，建议只画与执行异步系统调用的协程的信息，水平方向按时间轴进行组织。<font color="red"> 需要确定协程的具体定义之后再改一版</font>)
   7. 4.3.1节中的同步和异步系统调用有可能统一成`read!(fd, buffer).await;`和`read!(fd, buffer);`吗？<font color="red"> Rust不支持同名函数重载，即使是Rust宏也需要在字符串或者参数上进行区分，这种形式很难做到，使用 泛型 + trait 实现函数重载不能根据 await 来判断类型，仍然需要根据其他的方式进行区分</font>
   8. 4.3.2节：我认为同步和异步系统调用的内核实现函数都是对应一个协程；它们的区别在于，在执行完相应功能后通过用户态中断还是sret返回结果。(<font color="red"> 同步系统调用在内核中对应的协程是调度协程，这里需要讨论，怎样才算协程切换？只有通过共享调度器进行await的才算切换，因此调度协程通过sret到达用户态，内核的协程没有切换，用户态通过ecall到达内核态，对应的内核态执行流依然是调度协程。所以这里调度协程的功能不仅仅是调度，还包括了处理用户系统调用和中断异常。考虑换一个名字</font>)
2. 第五节开头讨论的TcpServer的三个组件的名字不好。“Msg Server”容易引起误解，“Msg Sender”说成“Msg Send”会与“Msg Recv”更对应一些。（<font color="green">采纳并已修改</font>）
3. 文字流畅性
   1. 英文的表达不够简洁和流畅，还需要好好润色。（目前可先不处理）
   1. 统一用词，用词要符合本专业通常的含义，尽量少用新词。
      * lightweight coroutine、coroutine, stackless coroutine（<font color="green">采纳并已修改</font>）
4. 针对上述意见的修改过程约定，要求每完成一条意见的修改或确定不采纳，在日志写修改说明。
