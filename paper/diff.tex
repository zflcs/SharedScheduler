...
%DIF DEL ../paper-base/sharedscheduler.tex   Mon Jul 29 17:01:18 2024
%DIF ADD sharedscheduler.tex                 Sat Aug  3 17:06:29 2024
...
%DIF <     tabsize     = 4,
%DIF <     frame       = trbl,
%DIF <     columns     = fullflexible,
%DIF <     commentstyle = \color[RGB]{0,128,0},
%DIF <     keywordstyle = \color[RGB]{0,0,255},
%DIF <     basicstyle   = \small\ttfamily,
%DIF <     % stringstyle  = \color[RGB]{148,0,209}\ttfamily,
%DIF <     rulesepcolor = \color{red!20!green!20!blue!20},
...
\DIFaddend The multi-threading model in the general operating systems is becoming insufficient in applications with increasing amounts of concurrency, due to the context-switching costs in the kernel multi-threading\DIFdelbegin \DIFdel{, and the kernel's inability to accurately schedule the user-level multi-threads for higher resource utilization}\DIFdelend . In this paper, a new concurrency model named COPS is proposed. \DIFdelbegin \DIFdel{We designed }\DIFdelend \DIFaddbegin \DIFadd{COPS uses }\DIFaddend a priority-based coroutine model as the smallest task unit to replace the \DIFdelbegin \DIFdel{multi-thread }\DIFdelend \DIFaddbegin \DIFadd{multi-threading }\DIFaddend model in large concurrency scenarios, and \DIFdelbegin \DIFdel{designed }\DIFdelend \DIFaddbegin \DIFadd{provides }\DIFaddend a unified priority-based scheduling framework for kernel space and user space coroutines. COPS \DIFdelbegin \DIFdel{utilize }\DIFdelend \DIFaddbegin \DIFadd{introduces coroutines as first-class objects in the OS to provide asynchronous I/O mechanism, which takes }\DIFaddend kernel coroutines as a bridge between I/O operations and devices \DIFdelbegin \DIFdel{to provide asynchronous I/O mechanisms}\DIFdelend \DIFaddbegin \DIFadd{and takes user coroutines as a bridge between applications and the OS's services respectively}\DIFaddend .

We \DIFaddbegin \DIFadd{design a prototype of web server based on COPS and }\DIFaddend conduct extensive experiments in an FPGA-based system to evaluate COPS. Results show that the proposed model achieves one to four times higher throughput while remains relatively lower overhead than that using the multi-threading model in the large concurrency applications.

%DIF <  【Q：基于优先级的调度如何能够保证高效的资源利用？有没有实验数据验证？】
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Multithreading technology, a classic concurrency model, has been widely supported by many operating systems and is widely used in various applications. However, as concurrency increases, the high system overhead and context-switching costs of the multithreading model gradually become unacceptable. The user-level multithreading, although reduces the context-switching overhead, but is only supported in userland and not perceived by the kernel, which leads to the kernel cannot accurately estimate the workload of tasks in the system nor execute more accurate scheduling policies to achieve higher system resource utilization. Based on the above situation, 
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < we propose COPS with the following characteristics: 1) It introduces coroutines into the kernel and applications as the smallest task unit, which is aimed to replace the traditional multithreading concurrency model. 2) It provides a unified priority-based scheduling framework for both user coroutines and kernel coroutines by attaching priorities to coroutines. 3) It uses kernel coroutines as the bridge between I/O operations and device interruptions to provide asynchronous I/O mechanism. Finally, we evaluate the characteristics of the COPS in a general web server scenario. In high-concurrency scenario, using coroutines can significantly reduce the context-switching overhead brought by the multithreading model, resulting in a throughput increase of 1.05-3.93x. Moreover, the priority-based scheduling ensures the effective resource utilization.
...
\DIFdelbegin %DIFDELCMD < \keywords{Multithreading, Concurrency, Coroutine.}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \keywords{Multi-threading, Concurrency, Coroutine.}
\DIFaddend 
...
%DIF < As concurrency and scalability increasing, a large-scale web server must accommodate tens thousands of simultaneous client connections without significant performance degradation. As of 2022, Google's servers are handling 883 billion requests per day \cite{google-search-statistics}, with an average of 8 million requests per second. For such a large amount of concurrent requests, multithreading model programs have achieved a certain degree of success, but as the demand gradually expands, the disadvantages of multithreading model are gradually exposed.
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < \begin{itemize}[leftmargin=*]
    %DIF < \item[1)] Threads, as a model of computation, are wildly nondeterministic \cite{Lee:EECS-2006-1}. Under the multithreading model, the uncertain execution sequence of programs leads to the uncertain accessing sequence of shared resources. Although reasonable synchronous exclusion mechanism can ensure the correct and safe accessing of shared resources in the concurrent environment, using multithreading is undoubtedly trying to obtain a deterministic goal in an uncertain way. In the concurrent scenario of web server, what needs to be done is receiving request package, processing it and sending response to the client. Using multithreading model blindly in such a deterministic resources accessing sequence scenario is very inappropriate.
    %DIF < \item[2)] The multithreading model is difficult to be combined with asynchronous I/O mechanism. Although it can be combined with the event-driven model underlying the operating system, it will increase the complexity of the operating system. For example, Linux provides system calls such as select and epoll \cite{Gammo2004ComparingAE} to support user-level asynchronous I/O tasks by reusing multiple I/O operations on a single thread. The epoll approach combines a single thread with an event-driven model, which leads to increased kernel complexity, requires interaction through the producer-consumer model and increases the overhead of synchronous mutual exclusion. 
    %DIFDELCMD < 

%DIFDELCMD <     %%%
%DIF < Windows' I/O Completion Ports (IOCP) \cite{alvinashcraft_io_2022} provides a similar I/O multiplexing mechanism, which is a truly asynchronous I/O operation by using callback functions. However, due to the asynchronous callback functions, it is very difficult to develop a correct program\cite{callbackhell}. 
    %DIFDELCMD < 

%DIFDELCMD <     
%DIFDELCMD <     %%%
%DIF < \item[3)] Threads are becoming expensive. The system threads need a complete set of hardware thread abstraction (stack, general register, program counter, etc.). Not only does it require more resources, but its context-switching is expensive which will bring serious performance degradation in large-scale concurrency scenario\cite{howell_cooperative_2002}. Although user-level threads can reduce context-switching overhead and increase flexibility, it cannot be perceived by the operating system which prevents performing fine-grained resource scheduling.
%DIF < \end{itemize}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < A lot of research has been done on the problem that the multithreading model is not suitable for large-scale concurrent web server scenario, but most still choose to continue using threads \cite{li_combining_2007, howell_cooperative_2002} which cannot fundamentally solve the problem. 
\DIFdelend The asynchronous I/O mechanism has proven to be efficient in large-scale concurrent web server scenarios. However, the design and implementation of the asynchronous I/O are difficult. OS not only needs to provide asynchronous I/O support for applications, but also needs to build a runtime for some of its own asynchronous tasks. There has been some work investigating asynchronous I/O mechanisms in OS. LXDs \DIFdelbegin \DIFdel{~}\DIFdelend \cite{narayanan2019lxds} developed a lightweight, asynchronous runtime environment in the kernel for cross-domain batch processing. Memif \DIFdelbegin \DIFdel{~}\DIFdelend \cite{lin2016memif} proposed a low-latency and low-overhead interface based on asynchronous and hardware-accelerated implementations. Lee \DIFdelbegin \DIFdel{~}\DIFdelend \textit{et al.} \DIFdelbegin \DIFdel{~}\DIFdelend \cite{lee2019asynchronous} introduced the asynchronous I/O stack (AIOS) to the kernel to reduce the I/O latency. Results showed that the performance of test applications are significantly improved. The above work has made progress in the research of asynchronous I/O mechanisms, but these methods are often independent of the kernel thread scheduler, resulting in a lack of versatility and scalability, and increasing the complexity of the kernel.

As an alternative \DIFdelbegin \DIFdel{to the multi-threading model, coroutines have attracted much attention in the design of system solving large-scale concurrent demands. DepFast~}\DIFdelend \DIFaddbegin \DIFadd{concurrency model, the concept of coroutines has not been widely adopted in modern programming languages in many decades. Currently, there is some renewal of interest in coroutines to explore the advantages of coroutines as an alternative to multi-threading. For example, DepFast }\DIFaddend \cite{luo_depfast_nodate} used coroutines in distributed arbitration systems; \DIFdelbegin \DIFdel{Capriccio~\mbox{%DIFAUXCMD
\cite{von_behren_capriccio_2003} }\hspace{0pt}%DIFAUXCMD
used cooperative user-level threads to achieve a scalable, large-scale web server}\DIFdelend \DIFaddbegin \DIFadd{Another resurgence of coroutines is in the scripting languages, such as Python and JavaScript}\DIFaddend .

In this paper, we rethink the concurrency model and asynchronous framework to find solutions that could meet the larger-scale and higher performance requirements. We propose COPS\footnote{The \textbf{CO} in COPS represents the coroutine, \textbf{P} represents the priority, and \textbf{S} represents scheduling. \DIFdelbegin \DIFdel{The entire operating system }\DIFdelend \DIFaddbegin \DIFadd{All tasks }\DIFaddend must be \DIFdelbegin \DIFdel{run }\DIFdelend \DIFaddbegin \DIFadd{carried out }\DIFaddend under COPS's management.}, a \\coroutine-based priority scheduling framework in OS. COPS improves the high context-switching overhead and solve the problem of uncertain sequence of accessing shared resources by using coroutines as the basic task unit. In addition, COPS draws on the existing research on asynchronous I/O mechanism, introduces coroutines into the kernel, \DIFaddbegin \DIFadd{provides coroutines as first-class objects }\DIFaddend and combines it with the asynchronous I/O mechanism to provide a coordinated and unified scheduling framework for all tasks in OS. 

\DIFaddbegin \DIFadd{The rest of the paper is organized as follows: we explain the fall and resurgence of coroutines, the coroutine facilities in Section \ref{section: Background}. We describe the design of COPS in Section \ref{section: design} and present the common asynchronous and concurrent programming coding patterns in Section \ref{section: Common usage patterns}. We evaluate the performance of COPS in Section \ref{section: Performance Evaluation}. We discuss the related work in Section \ref{section: Related Work} and conclude the paper in Section \ref{section: Conclusion}.
}


\DIFaddend \section{Background}
\DIFaddbegin \label{section: Background}
\DIFaddend 

\subsection{Coroutine}

\DIFdelbegin \DIFdel{Coroutine }\DIFdelend \DIFaddbegin \DIFadd{The concept of coroutines was firstly introduced in the early 1960's and constitutes one of the oldest proposals of general control abstraction. It }\DIFaddend is a lightweight concurrency abstraction that enable for the cooperative scheduling of multiple execution flow on a single system thread. Compared to processes or system threads, coroutines have lower resources \DIFdelbegin \DIFdel{requirement }\DIFdelend \DIFaddbegin \DIFadd{requirements }\DIFaddend and context-switching overhead. \DIFdelbegin \DIFdel{Modern programming languages , For example, c++ 20 \mbox{%DIFAUXCMD
\cite{C++20-coroutine}}\hspace{0pt}%DIFAUXCMD
, Go \mbox{%DIFAUXCMD
\cite{goroutines}}\hspace{0pt}%DIFAUXCMD
, Rust \mbox{%DIFAUXCMD
\cite{rosendahl2017green}}\hspace{0pt}%DIFAUXCMD
, Python \mbox{%DIFAUXCMD
\cite{python-coroutine}}\hspace{0pt}%DIFAUXCMD
, Kotlin \mbox{%DIFAUXCMD
\cite{kotlin-coroutines}}\hspace{0pt}%DIFAUXCMD
, etc. , all provide varying degrees of support for coroutines . Coroutine can be divided into the following two categories by the implementation}\DIFdelend \DIFaddbegin \DIFadd{However, coroutines has not been widely adopted in modern programming languages in many decades. The absence of coroutines can be partly attributed to the lack of a uniform view of the concept, which was never precisely defined. The fundamental characteristics of coroutines, shown as follows, are summarized in some literatures \mbox{%DIFAUXCMD
\cite{1980Coroutines}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend : 

\begin{itemize}[leftmargin=*]
    \item[1)] \DIFdelbegin \DIFdel{Stackful coroutine : It can generally be considered user-level threads. Each Stackful coroutine saves the function call chain and local variables in its own running stack space, which is allocated by runtime. Compared with system threads, Stackful coroutines optimize the overhead of context switching, but the effect is limited. }\DIFdelend \DIFaddbegin \DIFadd{The values of data local to a coroutine persist between successive calls;
    }\DIFaddend \item[2)] \DIFdelbegin \DIFdel{Stackless coroutine: It runs on a public stack. Normally, the local variables it uses are saved in the heap and used as needed.
Therefore, stackless coroutines do not need to allocate a fixed size of stack space, and the overhead of context switching is minimal.
}\DIFdelend \DIFaddbegin \DIFadd{The execution of a coroutine is suspended as control leaves it, only to carry on where it left off when control re-enters the coroutine at some later stage.
}\DIFaddend \end{itemize}

\DIFaddbegin \DIFadd{However, such a general definition leaves open relevant issues with respect to a coroutine construct which makes the support of coroutine facilities in programming languages different. Some implementation even contributed to the misconception of the expressive power of coroutines. Besides the absence of a precise definition of the concept, the other factors that greatly contributed to the discard of this interesting concept are the introduction of first-class continuation, and the adoption of multi-threading as a "de facto standard" concurrent construct \mbox{%DIFAUXCMD
\cite{2009Revisiting}}\hspace{0pt}%DIFAUXCMD
.
}

\DIFadd{Moura et al. \mbox{%DIFAUXCMD
\cite{2009Revisiting} }\hspace{0pt}%DIFAUXCMD
demonstrate that full coroutines have an expressive power equivalent to one-shot continuations and one-shot delimited continuations, which promotes the revival of coroutiness as a convenient general control abstraction. Nowdays, modern programming languages, For example, c++ 20 \mbox{%DIFAUXCMD
\cite{C++20-coroutine}}\hspace{0pt}%DIFAUXCMD
, Go \mbox{%DIFAUXCMD
\cite{goroutines}}\hspace{0pt}%DIFAUXCMD
, Rust \mbox{%DIFAUXCMD
...
, Kotlin \mbox{%DIFAUXCMD
\cite{kotlin-coroutines}}\hspace{0pt}%DIFAUXCMD
, etc., all provide varying degrees of support for coroutines.
}

\DIFaddend \subsection{\DIFdelbegin \DIFdel{Asynchronous Programming }\DIFdelend \DIFaddbegin \DIFadd{The Coroutine facilities }\DIFaddend in Rust}
\label{subsection: rust_async}

As a modern programming language designed by Mozilla, Rust is outstanding in terms of performance, security, and concurrent programming. \DIFdelbegin \DIFdel{Unlike Python and Java, which use background processes to maintain the runtime (such as using a garbage collection process for memory management), Rust has a relatively lightweight runtime. It provides relatively complete asynchronous programming support, allowing programmers to easily write asynchronous operations without having to deal with complex IO callbacks, which is very suitable for building IO bound applications}\DIFdelend \DIFaddbegin \DIFadd{Rust provides a powerful trait system to achieve the same expressive power as other object-oriented programming languages, such as providing abstraction, inheritance, and polymorphism}\DIFaddend . 

The \DIFdelbegin \DIFdel{support of Rust asynchronous programming is achieved by the three key components: 1) Future trait , 2) async/await syntax, 3) runtime library. The first two components help facilitate the development of asynchronous programs, while the runtime library enables asynchronous programs to run smoothly. }%DIFDELCMD < 

%DIFDELCMD < \begin{itemize}[leftmargin=*]
%DIFDELCMD <     \item[1)] %%%
\DIFdel{The future traits: Traits in Rust are similar to interface functions in other programming languages, defining abstract common behaviors. Future }\DIFdelend \DIFaddbegin \DIFadd{coroutine facilities in Rust is built on the trait system. Rust provides a special trait named }\textbf{\DIFadd{Future}} \DIFadd{in its core component. An object that implements the Future trait }\DIFaddend is an object with asynchronous behavior, \DIFdelbegin \DIFdel{and the future trait is the core of Rust asynchronous programming}\DIFdelend \DIFaddbegin \DIFadd{which is called as a coroutine}\DIFaddend . The poll function \DIFdelbegin \DIFdel{it specifies }\DIFdelend \DIFaddbegin \DIFadd{defined by the Future trait }\DIFaddend is used to drive the execution progress of \DIFdelbegin \DIFdel{asynchronous objects}\DIFdelend \DIFaddbegin \DIFadd{the coroutine}\DIFaddend . When the poll function returns Poll::Ready, it means that the \DIFdelbegin \DIFdel{asynchronous behavior }\DIFdelend \DIFaddbegin \DIFadd{coroutine }\DIFaddend has ended. On the contrary, returning Poll::Pending means that the \DIFdelbegin \DIFdel{asynchronous behavior }\DIFdelend \DIFaddbegin \DIFadd{coroutine }\DIFaddend cannot continue to be executed and needs to wait for the corresponding event to occur before it can continue. The \DIFdelbegin \DIFdel{future }\DIFdelend \DIFaddbegin \DIFadd{definition of the Future }\DIFaddend trait is shown in listing \ref{future_trait}\DIFdelbegin \DIFdel{,
    }%DIFDELCMD < \item[2)] %%%
\DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{. 
}

\begin{listing}[ht]
\caption{\DIFadd{Future trait.}}
\label{future_trait}
\begin{mdframed}
\begin{minted}[linenos=false,breaklines]{rust}
\DIFadd{pub trait Future }{
  \DIFadd{type Output;
  fn poll(self: Pin<}&\DIFadd{mut Self>, cx: }&\DIFadd{mut Context<'_>) -> Poll<Self::Output>;
}}
\DIFadd{enum Poll<T> }{
  \DIFadd{Ready(T),
  Pending,
...
\DIFadd{Besides implementing Future trait manually, Rust also provides }\DIFaddend async/await \DIFdelbegin \DIFdel{syntax: }\DIFdelend \DIFaddbegin \DIFadd{keywords to create coroutines and drive the execution of coroutines. }\DIFaddend The async keyword is \DIFdelbegin \DIFdel{syntactic sugar provided by Rust, }\DIFdelend used to modify functions, code blocks, and closures, converting them into anonymous futures. The await keyword is another syntactic sugar\DIFdelbegin \DIFdel{. When asynchronous behavior cannot continue, await will cause the current future }\DIFdelend \DIFaddbegin \DIFadd{, not only it can be used to execute the Future object, but also can be used }\DIFaddend to give up CPU usage \DIFaddbegin \DIFadd{when the asynchronous behavior cannot continue}\DIFaddend , allowing some other futures to execute.
\DIFdelbegin %DIFDELCMD < \item[3)] %%%
\DIFdel{The Rust runtime: The asynchronous execution of futures is guaranteed by Rust’s runtime. Multiple futures are combined into a task, and these tasks can be executed in a non-blocking sequence on a single thread. Typically, the scheduling of these tasks is determined by the Executor in the runtime, rather than the operating system scheduler. Therefore, this style of logic model is similar to green threads, and the operating system will not be aware of the Rust tasks in userland.
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < %%%
\DIFdelend 

\DIFdelbegin %DIFDELCMD < \begin{lstlisting}[label={future_trait},caption={Future trait}]
%DIFDELCMD < %%%
%DIFAUXCMD NEXT
\DIFmodbegin
\begin{lstlisting}[label={future_trait},caption={Future trait},alsolanguage=DIFcode]
%DIF < pub trait Future {
%DIF <     type Output;
%DIF <     fn poll(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output>;
%DIF < }
%DIF < enum Poll<T> {
%DIF <     Ready(T),
%DIF <     Pending,
...
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Through the Future trait and async/await keywords in Rust, the coroutines are provided as first-class objects to developers in the level of language. Moreover, Rust decouples the coroutine runtime from the definition of the coroutine, which means that developers can customize the coroutine runtime according to their own needs. Both the definition of coroutines and the decoupling property of coroutine runtime provides the possibility of introducing coroutines into the kernel not only the user space. Going even further, providing coroutines as first-class objects in the OS level is foreseeable.
}\DIFaddend 

\section{Design of COPS}
\DIFaddbegin \label{section: design}
\DIFaddend 

\DIFdelbegin \DIFdel{We choose to use the stackless coroutine to replace the traditional multithreading model in large-scale concurrent web server scenario, and we utilize Rust coroutines due to the strict checking mechanism of the Rust language compiler and the significant advantages in terms of memory safety. However, the operating system cannot directly perceived of the coroutine mechanism provided by the programming language. As learning more about Rust coroutines , we gradually realize the relationship between coroutines and asynchronous I/O mechanism. So we try to introduce coroutines into kernel to handle a wide variety of asynchronous tasks. This provides an opportunity to define a unified asynchronous task scheduling mechanism in the operating system. Therefore, we propose a coroutine-based priority scheduling framework perceived by the operating system, which can provide a coordinated and unified }\DIFdelend \DIFaddbegin \DIFadd{The design of COPS is tightly focused on improving the concurrency capability of the system. COPS take coroutines as the smallest task unit and provides coroutines as first-class objects in the OS. The kernel and application developers can directly use coroutines without any extra restrictions. It provides a unified priority-based }\DIFaddend scheduling framework for \DIFdelbegin \DIFdel{all tasks in the operating system and provide a unified asynchronous I/O framework to meet the requirement of high concurrency}\DIFdelend \DIFaddbegin \DIFadd{kernel space and user space coroutines}\DIFaddend . 
...
\DIFaddbegin \begin{figure}[h]
  ...
  \caption{\DIFaddFL{The architecture of COPS.}}
  \label{fig:arch}
  \Description{The architecture of COPS.}
\end{figure}

\DIFaddend Next, we will cover the design details of COPS in the rest of this section. We will first introduce the data structure related to the coroutine runtime provided in COPS to describe how to implement the scheduling of coroutines(\ref{section: Coroutine Runtime}). Next, we will introduce the \DIFdelbegin \DIFdel{concurrency of COPS(\ref{section: Concurrency}}\DIFdelend \DIFaddbegin \DIFadd{state transition model of the coroutines(\ref{section: state-transition}}\DIFaddend ). Then, we will \DIFdelbegin \DIFdel{introduce the state transition model of the coroutine (\ref{section: state-transition}}\DIFdelend \DIFaddbegin \DIFadd{describe the COPS API(\ref{section: cops api}}\DIFaddend ). Finally, we will describe the global cooperative scheduling mechanism provided by COPS(\ref{section: global-cooperative-scheduling}).

\DIFdelbegin %DIFDELCMD < \begin{figure}[h]
%DIFDELCMD <   \centering
%DIFDELCMD <   \includegraphics[width=\linewidth]{assets/arch.pdf}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The architecture of COPS.}}
  %DIFAUXCMD
%DIFDELCMD < \label{fig:arch}
%DIFDELCMD <   \Description{The architecture of COPS.}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Coroutine Runtime}
...
\DIFdelbegin %DIFDELCMD < \begin{lstlisting}[label={ccb},caption={Coroutine control block}]
%DIFDELCMD < %%%
%DIFAUXCMD NEXT
\DIFmodbegin
...
%DIFDELCMD < \label{section: Concurrency}
%DIFDELCMD < 
...
%DIF <  Despite the uncertainty introduced here, it is handled by the synchronous mutual exclusion mechanism inside COPS, which is completely transparent to the application.
}%DIFDELCMD < 
...
\DIFaddbegin \subsection{\DIFadd{COPS API}}
\label{section: cops api}

\DIFadd{According to the general definition of coroutines, the coroutines has three basic operators: }\textit{\DIFadd{create}}\DIFadd{, }\textit{\DIFadd{resume}} \DIFadd{and }\textit{\DIFadd{yield}}\DIFadd{. In Rust, the }\textit{\DIFadd{resume}} \DIFadd{and }\textit{\DIFadd{yield}} \DIFadd{operators are replaced by the poll function and the await keyword, respectively. Since the future must be driven by await keyword, it is not necessary to consider whether the }\textit{\DIFadd{yield}} \DIFadd{is placed in the right places. Certainly, we cannot avoid that the developer may accidentally use a tight loop in the coroutine, which will cause the coroutine to occupy the CPU for a long time. Pure cooperative scheduling frameworks are helpless against this dilemma. However, COPS can deal this dilemma through a global cooperative scheduling mechanism. This will be introduced in the section \ref{section: global-cooperative-scheduling}.
}

\DIFadd{COPS provides the }\textit{\DIFadd{spawn}} \DIFadd{operator to create a new coroutine. Besides, COPS also provides additional interfaces(}\textit{\DIFadd{current, wake\_up, set\_priority, alloc\_cpu}}\DIFadd{) to enhance the control capability of coroutines. The }\textit{\DIFadd{spawn}} \DIFadd{function create a new coroutine in the current process(kernel) and return the identity of the new coroutien. The first argument is the closure of the coroutine main body. The }\textit{\DIFadd{current}} \DIFadd{function returns the identity of the current coroutine. The }\textit{\DIFadd{wake\_up}} \DIFadd{function wakes up the coroutine with the specified identity. And the }\textit{\DIFadd{set\_priority}} \DIFadd{function sets the priority of the coroutine with the specified identity. The }\textit{\DIFadd{alloc\_cpu}} \DIFadd{function is used to apply for more CPU resource. Default, an application is only allocated one CPU. Developers can call the }\textit{\DIFadd{alloc\_cpu(1)}} \DIFadd{to apply for an additional CPU for the current application. Table \ref{tab:interface} shows the interfaces provided by COPS.
}

\DIFadd{Using the COPS API, developers can take advantage of coroutine at mostly without any restrictions, no matter in the kernel or user space.
}

\begin{table}[]
  \begin{tabular}{@{}cc@{}}
  \toprule
  \DIFaddFL{Function                     }& \DIFaddFL{Return Value }\\ \midrule
  \DIFaddFL{spawn(future, priority)      }& \DIFaddFL{cid          }\\
  \DIFaddFL{current()                    }& \DIFaddFL{cid          }\\
  \DIFaddFL{wake\_up(cid)                }&              \\
  \DIFaddFL{set\_priority(cid, priority) }&              \\
  \DIFaddFL{alloc\_cpu(cpu\_num)          }&              \\ \bottomrule
  \end{tabular}
  \caption{\DIFaddFL{Interface of COPS.}}
  \label{tab:interface}
  \vspace{-3em}
\end{table}

\DIFaddend \subsection{Global Cooperative Scheduling Mechanism}
...
Through the above mechanism, COPS provides an operating system aware coroutine-based scheduling framework\DIFaddbegin \DIFadd{, which can avoid one coroutine occupying CPU for a long time}\DIFaddend . Fig.~\ref{fig:flow} shows the code logic of COPS.
...
\section{\DIFdelbegin \DIFdel{Build program with }%DIFDELCMD < {%%%
\DIFdel{COPS}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{Common usage patterns}\DIFaddend }
\DIFaddbegin \label{section: Common usage patterns}
\DIFaddend 

In \DIFdelbegin \DIFdel{order to facilitate application developers to use COPS to build highly concurrent asynchronous programs, we provide a comprehensive set of programming interfaces and asynchronous system call interfaces. }%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{The Interface of COPS}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{In order to ensure that application development is compatible with traditional methods, we have adjusted the Unix-like runtime environment of the user process for compatibility, so that the main function is not executed immediately after the user process is initialized, but is wrapped into the main coroutine and added to the ready queue for unified scheduling. This means that after the user process is initialized, }\DIFdelend \DIFaddbegin \DIFadd{this section, we illustrate COPS use patterns for concurrent and asynchronous programming. Both the patterns are broadly applicable. COPS provides an environment that }\DIFaddend all tasks exist in the form of coroutines\DIFdelbegin \DIFdel{, in a cooperative execution environment}\DIFdelend . What the main \DIFdelbegin \DIFdel{coroutine }\DIFdelend \DIFaddbegin \DIFadd{body of application }\DIFaddend needs to do is using the \DIFdelbegin \DIFdel{interface of COPS to create different coroutines }\DIFdelend \DIFaddbegin \textit{\DIFadd{spawn}} \DIFadd{function to create coroutines with specific functionality}\DIFaddend . The running of coroutines is transparently driven by the coroutine runtime provided by COPS.
\DIFdelbegin \DIFdel{COPS provides the interfaces to application developers in Table \ref{tab:interface}.
}%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Interface of COPS}}
  %DIFAUXCMD
%DIFDELCMD < \label{tab:interface}
%DIFDELCMD <   \begin{tabular}{|c|c|}
%DIFDELCMD <     \hline
%DIFDELCMD <     %%%
\textbf{\DIFdelFL{Interface}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Description}} %DIFAUXCMD
%DIFDELCMD < \\                   
%DIFDELCMD <     \hline
%DIFDELCMD <     %%%
\DIFdelFL{spawn(future, prio) }%DIFDELCMD < &\makecell{Create a new coroutine \\ with specific priority.} \\
%DIFDELCMD <     \hline
%DIFDELCMD <     %%%
\DIFdelFL{getcid(void) }%DIFDELCMD < & \makecell{Get the Id of current \\ coroutine.} \\
%DIFDELCMD <     \hline
%DIFDELCMD <     %%%
\DIFdelFL{wake(cid) }%DIFDELCMD < & \makecell{Wake up the specific \\ coroutine.} \\
%DIFDELCMD <     \hline
%DIFDELCMD <     %%%
\DIFdelFL{reprio(cid, prio) }%DIFDELCMD < & \makecell{Adjust the priority of \\ the target coroutine.} \\
%DIFDELCMD <     \hline
%DIFDELCMD <     %%%
\DIFdelFL{alloc\_cpu(void) }%DIFDELCMD < & \makecell{Allocate more cpu to support \\ a higher degree of concurrency.}\\
%DIFDELCMD <     \hline
%DIFDELCMD < 
...
\DIFdelbegin %DIFDELCMD < \begin{lstlisting}[label={system call}, caption={System call interface of read().}]
%DIFDELCMD < %%%
%DIFAUXCMD NEXT
\DIFmodbegin
\begin{lstlisting}[label={system call}, caption={System call interface of read().},alsolanguage=DIFcode]
%DIF <     read!(fd, buffer, cid); // Async call
%DIF <     read!(fd, buffer); // Sync call
\end{lstlisting}
\DIFmodend %DIFAUXCMD
%DIFDELCMD < \end{lstlisting}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{Algorithm \ref{alg: asynchronous programming} shows how to use the asynchronous system call interface for asynchronous programming. In the async\_read function, it calls the }\textit{\DIFadd{read!}} \DIFadd{system call interface for asynchronous reading. If there is no data in the kernel buffer, the async\_read coroutine will give up the CPU. Once there is data in the kernel buffer, the async\_read coroutine will be waked up.
}

\begin{algorithm}[!h]
  \caption{\DIFadd{Asynchronous Programming}}
  \label{alg: asynchronous programming}
  \begin{algorithmic}[1]
    \Function{main}{}
      \State \DIFadd{$spawn(|| async\_read, 0)$;
		}\EndFunction

    \Function{async\_read}{}
      \State \DIFadd{$buf \gets [0; buf\_len]$;
      }\State \DIFadd{$current\_cid \gets current()$;
      }\State \DIFadd{$read!(buf.ptr, buf.len, current\_cid)$;
    }\EndFunction
  \end{algorithmic}
\end{algorithm}

\begin{listing}
  \caption{\DIFadd{System call interface of read().}}
  \label{listing: system call}
  \begin{mdframed}
  \begin{minted}[linenos=false,breaklines]{rust}
  \DIFadd{read!(fd, buffer, cid); // Async call
...
{%DIFAUXCMD
\DIFdelFL{Configuration of evaluation}}
  %DIFAUXCMD
%DIFDELCMD < \label{tab:cfg}
%DIFDELCMD <   %%%
...
\DIFdelbegin %DIFDELCMD < \begin{enumerate}[leftmargin=*]
%DIFDELCMD <     \item %%%
...
\begin{figure}[ht]
  \DIFaddbeginFL \Description[]{Throughput and message latency of different priority connections.}
	\DIFaddendFL \centering
	...
	\DIFdelbeginFL %DIFDELCMD < \Description{Throughput and message latency of different priority connections.}
%DIFDELCMD < %%%
...
\begin{figure}[ht]
    \centering
    \DIFdelbeginFL %DIFDELCMD < \subcaptionbox{Core-2\label{subfig:prio-core2}}[1.0\linewidth]
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \subcaptionbox{Core-2\label{subfig:prio-core2}}[0.49\linewidth]
    \DIFaddendFL {
      \includegraphics[width=\linewidth]{assets/prio-core2.pdf}
    }
    \DIFdelbeginFL %DIFDELCMD < \subcaptionbox{Core-4\label{subfig:prio-core4}}[1.0\linewidth]
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \subcaptionbox{Core-4\label{subfig:prio-core4}}[0.49\linewidth]
    \DIFaddendFL {
