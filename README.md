...
同步读写环的性能对比结果如图6所示, 其中 AsyncOS-228 表示在 AsyncOS中实现同步读写环并在环中传递长度为228字节的数据，SharedScheduler-912表示在共享调度器中实现同步读写环并在环中传递长度为912字节的数据。
...
为了衡量协程与线程在切换上的开销，我们分别用线程模型和协程模型实现了 Server Process 的三个部分。
