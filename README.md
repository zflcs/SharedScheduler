...
### 7.1 串行的 pipe 环
仿照 AsyncOS 的串行管道环实验，我们将若干个管道的读写端首尾相连，形成一个环状结构，如图5所示，我们将一读一写封装成一个协程，然后将第一个协程叫做client端，负责第一次写入输入来启动整个管道环，其余的负责传递消息的协程叫做server端，client经过第一次点火写入之后，由各个server端协程进行消息传递，最终回到client的读端。
