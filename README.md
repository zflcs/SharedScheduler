...
我们通过 vDSO 的形式让内核和所有用户进程能够访问到 SharedScheduler 中的代码，但在这个过程中，遇到了很多问题，最终以比较笨拙的方式使得能够勉强满足需求。

#### 5.1.1 笨拙的共享方式

我们遇到的第一个问题是 Rust 无法不支持编译出 riscv64gc-unknow-none-elf 架构的共享库文件，因此只能以 bin crate 的形式将其编译成二进制文件，由内核将其加载进自己的地址空间中进行维护。之后，在创建进程时，将 SharedScheduler 的地址映射添加进用户进程地址空间中，使得内核和用户进程能够访问其中的接口。但内核和用户不知道这些接口的入口地址，只能通过解析符号表查找对应的符号才可以使用。

#### 5.1.2 堆与 Executor

同样，这个问题也是由于 Rust 语言的特点所带来的，Rust 使用默认的全局分配器（global allocator）或者显示的指明所用的分配器在堆上进行分配、释放内存。因此，有两种方式能够动态的完成对数据的分配和释放。第一种方式是显示的将堆指针当作参数传递给 SharedScheduler 接口；第二种方式是采用某种方式使得全局分配器能够动态的指向内核和进程各自的堆。

在这里，我们将内核和用户进程的堆映射到同一个虚拟地址，实现了[第二种方式](https://github.com/zflcs/rCore-N/blob/main/lib_so/src/lang_items.rs#L43)。

#### 5.1.3 共享库

在标准库环境下，若在 Rust 中使用共享库，需要先声明外部函数，最后再进行链接。但现实却是我们一方面无法生成共享库文件，另一方面 riscv64gc-unknow-none-elf 交叉编译工具链也不支持链接共享库。最终，我们将[静态库和链接脚本进行相互配合](https://github.com/zflcs/rCore-N/blob/main/lib_so/src/lib.rs#L115)，使得在编写用户程序时能够像使用静态库中的函数一般使用 SharedScheduler 暴露出来的共享库。

这种方式体现了模块化的特点，并且通过不同的 feature 可以向内核和用户暴露出不同的接口，见下表。
...
内核和所有的用户进程能够访问 SharedScheduler 暴露的共享库。但这些操作最终会落实到对其内部的 Executor 数据结构的操作上。为了避免在内核和用户库中重复定义 Executor 数据结构，我们将 Executor 定义在上述提到的静态库中，并为其实现某些成员方法。

按照 Rust 的特性，仅仅在内核和用户进程中创建 Executor 而不使用其成员方法，那么内核和用户程序编译生成的 ELF 文件中将不会包含这些成员方法。而在 SharedScheduler 中，我们通过调用 Executor 的成员方法完成对它内部的操作。这样一种特性，使得我们既能保留住共享的特点，又能方便的实现模块化的协程调度框架，通过不同的 feature 使用不同的协程调度算法，见图 4。
